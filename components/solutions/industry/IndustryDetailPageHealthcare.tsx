...
/**
 * Healthcare & Pharma only uses this subset of the shared
 * IndustryProduct["icon"] union (see @/data/industries/types).
 */
type HealthcareProductIcon =
  | "camera"
  | "access"
  | "ups"
  | "solar"
...
const productIconMap: Record<HealthcareProductIcon, React.ReactNode> = {
  camera: <VideocamRoundedIcon />,
  access: <FingerprintRoundedIcon />,
...
export default function IndustryDetailPageHealthcare({ data }: Props) {
  return (
    <Box component="main">
      <Hero data={data} />
      <Stats data={data} />
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="Security and infrastructure gaps in healthcare and pharma show up in specific places. Here's where we focus."
        />
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "linear-gradient(to bottom, #f6faff, #ffffff)" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="Every facility gets its own mix — this is the full toolkit we draw from."
        />
        ...
        >
          {data.products.map((p) => (
            ...
            >
              ...
              >
                {productIconMap[(p as IndustryProduct).icon as HealthcareProductIcon]}
              </Box>
              <Box>
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          title="From facility visit to support line"
          description="The same four steps whether it's a single clinic or a multi-facility hospital group."
        />
        ...
        >
          <Box>
            <Typography sx={{ color: "#fff", fontWeight: 900, fontSize: { xs: "20px", md: "26px" }, mb: 1, lineHeight: 1.25 }}>
              Ready to equip your {data.title.toLowerCase()} facility?
            </Typography>
            <Typography sx={{ color: "rgba(255,255,255,0.68)", fontSize: "13px", maxWidth: 480, lineHeight: 1.65 }}>
              Tell us how many blocks or wards you're working with — we'll come back with a plan and a quote.
            </Typography>
          </Box>
