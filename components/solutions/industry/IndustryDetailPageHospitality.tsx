...
/**
 * Hospitality only uses this subset of the shared
 * IndustryProduct["icon"] union (see @/data/industries/types).
 */
type HospitalityProductIcon =
  | "camera"
  | "fire"
  | "wifi"
  | "network"
...
const productIconMap: Record<HospitalityProductIcon, React.ReactNode> = {
...
export default function IndustryDetailPageHospitality({ data }: Props) {
  return (
    <Box component="main">
      <Hero data={data} />
      <Stats data={data} />
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="Security and infrastructure gaps in hospitality show up in specific places. Here's where we focus."
        />
        ...
        >
          {data.products.map((p) => (
            ...
            >
              ...
              >
                {productIconMap[(p as IndustryProduct).icon as HospitalityProductIcon]}
              </Box>
              <Box>
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          title="From property visit to support line"
          description="The same four steps whether it's a single boutique hotel or a multi-property chain."
        />
        ...
        >
          <Box>
            <Typography sx={{ color: "#fff", fontWeight: 900, fontSize: { xs: "20px", md: "26px" }, mb: 1, lineHeight: 1.25 }}>
              Ready to equip your {data.title.toLowerCase()} property?
            </Typography>
            <Typography sx={{ color: "rgba(255,255,255,0.68)", fontSize: "13px", maxWidth: 480, lineHeight: 1.65 }}>
              Tell us how many rooms or properties you're working with — we'll come back with a plan and a quote.
            </Typography>
          </Box>
