...
/**
 * Retail & Office only uses this subset of the shared
 * IndustryProduct["icon"] union (see @/data/industries/types).
 */
type RetailOfficeProductIcon =
  | "camera"
  | "fire"
  | "wifi"
  | "ups"
...
const productIconMap: Record<RetailOfficeProductIcon, React.ReactNode> = {
  camera: <VideocamRoundedIcon />,
  fire: <LocalFireDepartmentRoundedIcon />,
  wifi: <WifiRoundedIcon />,
  ups: <BatteryChargingFullRoundedIcon />,
  pos: <PointOfSaleRoundedIcon />,
  server: <DnsRoundedIcon />,
  solar: <SolarPowerRoundedIcon />,
  itequipment: <DevicesRoundedIcon />,
  lockers: <LockOpenRoundedIcon />,
  audio: <CampaignRoundedIcon />,
  metaldetector: <SensorDoorRoundedIcon />,
  telephone: <DialpadRoundedIcon />,
  conference: <MeetingRoomRoundedIcon />,
};
...
export default function IndustryDetailPageRetailOffice({ data }: Props) {
  return (
    <Box component="main">
      <Hero data={data} />
      <Products data={data} />
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="Security and infrastructure gaps in retail and office environments show up in specific places. Here's where we focus."
        />
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "linear-gradient(to bottom, #f6faff, #ffffff)" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="Every store or office gets its own mix — this is the full toolkit we draw from."
        />
        ...
        >
          {data.products.map((p) => (
            ...
            >
              ...
              >
                {productIconMap[(p as IndustryProduct).icon as RetailOfficeProductIcon]}
              </Box>
              <Box>
    ...
    <Box sx={{ py: { xs: 6, md: 9 }, background: "#fff" }}>
      <Container maxWidth="xl">
        <SectionHeading
          ...
          description="The same four steps whether it's a single outlet or a multi-location chain."
        />
        ...
        >
          <Box>
            <Typography sx={{ color: "#fff", fontWeight: 900, fontSize: { xs: "20px", md: "26px" }, mb: 1, lineHeight: 1.25 }}>
              Ready to equip your {data.title.toLowerCase()} location?
            </Typography>
            <Typography sx={{ color: "rgba(255,255,255,0.68)", fontSize: "13px", maxWidth: 480, lineHeight: 1.65 }}>
              Tell us how many stores or floors you're working with — we'll come back with a plan and a quote.
            </Typography>
          </Box>
